...
create table if not exists public.customer_addresses (
  ...
  label text not null default 'Home', full_name text not null, phone text not null, address text not null,
...
create table if not exists public.customer_wishlist (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  product_name text not null, price text not null, created_at timestamptz not null default now(), unique(user_id, product_name)
);
create table if not exists public.customer_orders (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references auth.users(id) on delete cascade,
  items jsonb not null default '[]'::jsonb, total numeric not null default 0,
...
drop policy if exists "Users view own orders" on public.customer_orders;
create policy "Users view own orders" on public.customer_orders for select using (auth.uid() = user_id);
drop policy if exists "Users create own orders" on public.customer_orders;
